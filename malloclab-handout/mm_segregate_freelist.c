...
static void set_as_freelistp(void *bp, size_t sizeclass);
static void print_freelist_arg(void *fp);
static void print_freelistarray();
static void *get_freelistp(void *bp);
...
static void *find_fit(size_t asize, void *heap_listp){
    ...
    for (size_t size_class=get_sizeclass(asize); size_class < n_sizeclass; size_class++){
        free_listp = free_listarray[size_class];
...
static void put_free(void *bp)
{
    ...
    void *free_listp = free_listarray[sizeclass];

    if (free_listp == NULL){
        /* The free list is empty */
        set_as_freelistp(bp, sizeclass);
        return;
    }
...
static void remove_free(void *bp){
    ...
    if (pred_bp == NULL)
        set_as_freelistp(succ_bp, sizeclass);
...
/*
 * get_freelistp - Given block pointer bp, 
 *     return the head pointer of its corresponding free_list.
 */
static void *get_freelistp(void *bp){
    size_t size = GET_SIZE(HDRP(bp));
    return free_listarray[get_sizeclass(size)];
}
...
static size_t get_sizeclass(size_t size){
    ...
    return MIN(MAX(0, (int) (size_log2 - 6)), n_sizeclass-1);
}
...
static int isin_free(void *bp){
    void *free_listp = get_freelistp(bp);
    if (free_listp == NULL)
        return 0;
